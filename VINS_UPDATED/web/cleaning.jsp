...
<title>🧼 Cleaning - Smart Grocery</title>
<style>
...
/* HERO */
.hero {height:220px;display:flex;flex-direction:column;justify-content:center;align-items:center;background:linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.4)),url('https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=1200&q=80');background-size:cover;background-position:center;color:white;text-align:center;}
...
<div class="nav">
    ...
    <input type="text" id="searchInput" placeholder="🔍 Search cleaning supplies..." onkeyup="filterProducts()">
    <div class="nav-right">
...
try {
    ...
    rs = st.executeQuery("SELECT * FROM products WHERE category='cleaning' ORDER BY id DESC");
...
%>
        <div class="product-card" data-name="<%= rs.getString("name").toLowerCase() %>">
            ...
            <form class="add-cart-form" method="post" action="addtocart.jsp">
                ...
                <input type="hidden" name="referer"    value="cleaning.jsp">
...
<% if(productCount == 0) { %>
    <div class="empty">
        <h3>🧼 No Cleaning Products Yet</h3>
