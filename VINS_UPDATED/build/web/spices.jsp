...
<title>🌶️ Spices - Smart Grocery</title>
<style>
...
/* HERO */
.hero {height:220px;display:flex;flex-direction:column;justify-content:center;align-items:center;background:linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.4)),url('https://images.unsplash.com/photo-1596040033229-a9821ebd058d?auto=format&fit=crop&w=1200&q=80');background-size:cover;background-position:center;color:white;text-align:center;}
...
<div class="nav">
    ...
    <input type="text" id="searchInput" placeholder="🔍 Search spices..." onkeyup="filterProducts()">
    <div class="nav-right">
...
try {
    ...
    rs = st.executeQuery("SELECT * FROM products WHERE category='spices' ORDER BY id DESC");
...
%>
        <div class="product-card" data-name="<%= rs.getString("name").toLowerCase() %>">
            <img src="<%= rs.getString("image") %>" alt="<%= rs.getString("name") %>" class="product-img"
                 onerror="this.src='https://via.placeholder.com/120x120/f0f8ff/87ceeb?text=🌶️'">
            ...
            <form class="add-cart-form" method="post" action="addtocart.jsp">
                ...
                <input type="hidden" name="referer"    value="spices.jsp">
